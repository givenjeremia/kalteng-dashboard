...
class DepartementController extends Controller
{
    ...
    public function tableDataAdmin(){
        if (request()->ajax())
        {
            $departements = Departement::orderBy('pkid','desc')->get();
            $counter = 1;
    ...
    {
        try {
            return response()->json(array('status' => 'success','msg' =>  view('pages.departement.modal.create')->render()), 200);
        } catch (\Throwable $e) {
    ...
    {
        try {
            $data= Departement::where('uuid',$id)->firstOrFail();
        
            return response()->json([
                'status' => 'success',
                'msg'    => view('pages.departement.modal.update', compact('data'))->render()
            ], 200);
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $departement = Departement::where('uuid', $id)->firstOrFail();
