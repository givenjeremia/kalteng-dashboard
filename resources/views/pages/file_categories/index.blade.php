...
@section('title','File Categories')

@section('toolbar')
@include('components/toolbar',['title' => 'File Categories'])
@endsection
...
<script>
    $(document).ready(function() {
        var datatable = $('#kt_table').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: "{{ route('file-categories.index') }}",
            columns: [
                {
                    data: 'No',
                    name: 'No',
...
<script>
    $('#btnTambah').on('click',function (e){
        e.preventDefault();
        $.ajax({
            url: "{{ route('file-categories.create') }}"
            , method: "GET"
...
<script>
    function showModalUpdate(data){
        let url = "{{ route('file-categories.edit', ':id') }}".replace(':id', data)
        $.ajax({
            url: url,
            method: "GET",
            success: function(response) {
    ...
    function deleteData(data){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                let url = "{{ route('file-categories.destroy', ':id') }}".replace(':id', data)
                $.ajax({
                    url: url,
                    method: "DELETE",
                    data: {
